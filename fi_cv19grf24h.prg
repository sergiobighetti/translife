...
cGrf= "['DiaHora', 'Assint', 'Leve', 'Grave', 'OMT' ]"

cBase = ''
Scan All
   xGrf = "['Dia "+ Transform(dia,'@L 99') +' '+ Transform(HORA,'@L 99')+":00-"+Transform(HORA,'@L 99')+":59'" + ','+ Transform(cv19_assint)+ ','+ Transform(cv19_leve)+ ','+ Transform(cv19_grave)+ ','+ Transform(nTotNSU)  + ']'
   cGrf = cGrf + ','+xGrf
Endscan
  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      function drawStacked() {
        ...
        var options = {
          title: '<<TRANSFORM(DATETIME())>>) <<TRANSFORM(nTotGeralCOR+nTotGeralNSU)>> atendimentos nas ultimas 24 horas sendo <<nTotGeralCOR>> CoronaVirus e <<nTotGeralNSU>> OMT',
          subtitle: '<<cSubTit>>',
          chartArea: {
            width: '70%'
          },
          isStacked: true,
          height: 800,
          colors: ['#87CEFA', '#FFFF00', '#ff4500', '#E6E6FA'],
          hAxis: {
            title: '<<cSubTit>>',
            minValue: 0,
          },
