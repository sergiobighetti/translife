FUNCTION  fi_z_rastreia()

LOCAL cAlsDIF, cAlsOLD, cArqDIF, cArqLOG, cArqOLD, cData, dDataAnt, nSel, tDate, cSAFETY

cArqLOG = '\\192.168.0.45\bdmem\RASTREIA.LOG'

cArqOLD = '\\192.168.0.45\bdmem\CTR_O_L_D'
* cArqOLD = 'c:\tmp\CTR_O_L_D.DBF'
cAlsOLD = 'CTR_O_L_D'

cArqDIF = '\\192.168.0.45\bdmem\CTR_D_I_F'
...
IF FILE(cArqLOG)
   ...
   IF !EMPTY(cData)
      ...
      IF (DATE() - dDataAnt) > 0
         SELECT idContrato AS ID, nome_responsavel AS razao, auditoria FROM CONTRATO WHERE situacao='ATIVO' ORDER BY idContrato INTO TABLE (cArqOLD)
      ...
      WHERE       aa.situacao='ATIVO' AND NOT aa.nome_responsavel == bb.razao ;
      INTO CURSOR (cLog)
