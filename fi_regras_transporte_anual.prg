...
#DEFINE DF_PLANO_TRANSP_ANUAL  22
#DEFINE DF_COD_TP_ATEND_TRANSP 14

LPARAMETERS cChv, dRef, lDispNegativo, nQual


nQual = IIF( TYPE( 'nQual')='N', nQual, 1 )

IF nQual = 1
   nQualProd = DF_PLANO_MEDICA_MAIS
ELSE
   nQualProd = DF_PLANO_TRANSP_ANUAL 
ENDIF
...
IF FI_TEM_PROD_GARANTIA( cChv, nQualProd  )
   ...
   WHERE       CONTRATO_GARANTIAS.idContrato == nCtrl AND ;
               CONTRATO_GARANTIAS.idProd == nQualProd ;
   INTO CURSOR CLV_ARG_GRT
	...
	IF _TALLY > 0
	   ...
	   IF !EMPTY(dVigencia)
         ...
         cWheClas = IIF( nQual=1, "ATENDIMENTO.medclassificacao ='CONS'", "ATENDIMENTO.ateclassificacao'TRANSPORTE'" )
	      ...
	      IF PTAB( LEFT( cChv, 10 ), 'HSTATEND', 'PACCODIGO', .t. )
            ...
            cWheClas = STRTRAN( cWheClas, "ATENDIMENTO.", "HSTATEND." )
	         ...
	         SCAN WHILE LEFT(HSTATEND.paccodigo,10) == LEFT(cChv,10)
	            ...
	            IF HSTATEND.situacao  == 2 AND ;                 && ENCERRADO
                  ...
                  IF nQual=1 OR ( nQual=2 AND HSTATEND.codatendimento == DF_COD_TP_ATEND_TRANSP )
   	               nUtilizado = nUtilizado + 1
...
FUNCTION FI_TEM_PROD_GARANTIA( cKey, nQualProd   )
